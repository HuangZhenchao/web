...
function set_curl_config($dataarray) {
	...
	// 设置cURL允许执行的最长秒数
	if($dataarray['CURLOPT_TIMEOUT']) {		
		curl_setopt ( $ch, CURLOPT_TIMEOUT, $dataarray['CURLOPT_TIMEOUT'] );
	}
	// 要求CURL返回数据
	if($dataarray['CURLOPT_RETURNTRANSFER']) {		
		curl_setopt ( $ch, CURLOPT_RETURNTRANSFER, $dataarray['CURLOPT_RETURNTRANSFER'] );
	}
	//开启curl post
	if($dataarray['CURLOPT_POSTFIELDS']) {
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, $dataarray['CURLOPT_POSTFIELDS']);  
	}
	// 执行请求
	$result = curl_exec ( $ch );	
