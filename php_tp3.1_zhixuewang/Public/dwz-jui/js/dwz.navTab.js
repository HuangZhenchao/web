...
var navTab = {
	...
	_scrollCurrent: function(){ // auto scroll current tab
		...
		//滚动栏宽度大于选项卡总长
		//向前滚动，显示最后一个
		//当前页在第一个显示的选项卡前，被隐藏。则从当前选项卡开始显示
		//当前页在最后一个显示的选项卡后，被隐藏。则向前滚动
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
				...
				if (op.external || url.isExternalUrl()) {
					...
					$panel.ajaxUrl({
						type:op.type, url:url, data:op.data, callback:function(){
							navTab._loadUrlCallback($panel);//回调函数，设置layoutH，添加取消按钮的事件
						}
					});
				}
			}
		...
		//选项卡点击事件的重置、关闭按钮点击事件的重置、更多列表的重置
		this._init();
		//滚动到当前页
		this._scrollCurrent();
		//设置当前navTab选项卡url属性
