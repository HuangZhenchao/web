...
class Flag{
	 ...
	 * 模块列表
	 */
	public function getlist_flag($fieldjson){
	...
	//搜索关键词
		...
		$sqlstr="select a.*,count(b.id) as flag_blog_count from flag a left join blog b on (a.id=b.flag1 or a.id=b.flag2) where ".$fieldstr." GROUP BY a.id order by flag_blog_count desc";				
