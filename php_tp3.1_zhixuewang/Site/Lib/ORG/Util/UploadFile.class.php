...
class UploadFile {//类定义开始
     ...
     * @access public
     +----------------------------------------------------------
     */
    public function __construct($maxSize='',$allowExts='',$allowTypes='',$savePath='',$saveRule='',$noExts='') {
        ...
        if(!empty($noExts)) {
            if(is_array($noExts)) {
                $this->noExts = array_map('strtolower',$noExts);
            }else {
                $this->noExts = explode(',',strtolower($noExts));
            }
        }
        if(!empty($allowTypes)) {
    ...
    public function upload($savePath ='') {
        ...
        // 检查上传目录
        if($savePath)
        $savePath_iconv=iconv("UTF-8","GBK",$savePath);
        if(!is_dir($savePath_iconv)) {
            // 检查目录是否编码后的
            if(is_dir(base64_decode($savePath_iconv))) {
                $savePath_iconv	=	base64_decode($savePath_iconv);
            }else{
                // 尝试创建目录
                if(!mkdir($savePath_iconv)){
                    $this->error  =  '上传目录'.$savePath.'不存在';
                    return false;
                }
            }
        }else {
            if(!is_writeable($savePath_iconv)) {
    ...
    protected function error($errorNo) {
         switch($errorNo) {
            case 1:
                $this->error = '上传的文件超过了max_filesize 选项限制的值';
                break;
            case 2:
    ...
    private function checkExt($ext) {
        ...
        if(!empty($this->noExts))
            return !in_array(strtolower($ext),$this->noExts);
        return true;
    }
